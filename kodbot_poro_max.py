...
def send_broadcast(text, parse_mode="Markdown"):
    ...
    with get_db() as conn:
        user_ids = [u["user_id"] for u in conn.execute("SELECT user_id FROM users").fetchall()]
        ...
        try:
            ...
            with counter_lock:
                if res.get("ok"):
                    ...
                    # اگر کاربر بات را بلاک کرده، از DB حذف کن یا نادیده بگیر
                    err_code = res.get("error_code", 0)
                    if err_code in (403, 400):
                        counter["fail"] += 0  # خاموشانه skip
                    else:
                        counter["fail"] += 1
        except Exception:
            with counter_lock:
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        executor.map(send_to_user, user_ids)
...
def handle_delete_bot_final(query: dict, bot_id: int):
    ...
    # FIX: حذف کل دایرکتوری ربات (نه فقط فایل اصلی)
    if bot["file_path"]:
        bot_dir = Path(bot["file_path"]).parent
        if bot_dir.exists() and bot_dir != BOTS_DIR:
            try:
                shutil.rmtree(str(bot_dir), ignore_errors=True)
            except Exception:
                pass
        elif os.path.exists(bot["file_path"]):
            try:
...
def start_bot_process(bot_id: int, file_path: str, bot_name: str, user_id: int) -> Optional[str]:
    try:
        ...
        lf.close()  # FIX: بستن FD والد - فرزند FD خودش را نگه می‌دارد
...
def stream_logs(bot_id: int, user_id: int, bot_name: str):
    ...
    MIN_SEND_INTERVAL = 30   # FIX: حداقل 30 ثانیه بین هر ارسال فایل
    last_send_time = 0
    ...
    while True:
        ...
        try:
            ...
            # FIX: بررسی throttle + تغییر واقعی
            content_hash = hashlib.md5(new_content.encode("utf-8", errors="ignore")).hexdigest()
            if content_hash == last_send_hash:
                continue
            now_ts = time.time()
            if now_ts - last_send_time < MIN_SEND_INTERVAL:
                continue  # هنوز زود است - لاگ جمع می‌شود و دفعه بعد ارسال می‌شود
...
def handle_callback_query(update: dict):
    ...
    # پنل مدیریت
        ...
        with get_db() as conn:
            t = conn.execute("SELECT user_id, status FROM tickets WHERE id=?", (ticket_id,)).fetchone()
        if t and t["status"] == "closed":
            answer_callback(query["id"], "این تیکت قبلاً بسته شده است.", True)
        else:
            ...
            # FIX: اطلاع به سایر مدیران که این تیکت در حال پاسخ است
            _notify_admins(f"🎫 مدیر `{user_id}` در حال پاسخ به تیکت *#{ticket_id}* است.", exclude=user_id)
    elif data.startswith("admin_close_ticket:"):
        ticket_id = int(data.split(":")[1])
        with get_db() as conn:
            t = conn.execute("SELECT user_id FROM tickets WHERE id=?", (ticket_id,)).fetchone()
            conn.execute("UPDATE tickets SET status='closed', replied_by=? WHERE id=?", (user_id, ticket_id,))
            conn.commit()
        answer_callback(query["id"], "✅ تیکت بسته شد.", True)
        # FIX: اطلاع به کاربر و سایر مدیران
        if t:
            send_message(t["user_id"], f"✅ تیکت #{ticket_id} شما توسط تیم پشتیبانی بسته شد.")
        _notify_admins(f"🔒 تیکت *#{ticket_id}* توسط مدیر `{user_id}` بسته شد.", exclude=user_id)
...
def handle_message(update):
    ...
    if st == "deploy_waiting_name":
        ...
        for admin_id in get_admin_ids():  # FIX: نه ADMIN_IDS استاتیک
            send_message(
                admin_id,
        ...
        _notify_admins(f"🎫 تیکت *#{ticket_id}* توسط مدیر `{user_id}` پاسخ داده شد.\n📝 {text[:100]}", exclude=user_id)
        clear_state(user_id)
...
_channel_verify_cache: Dict[int, tuple] = {}  # {user_id: (result, timestamp)}
_CHANNEL_CACHE_TTL = 300  # 5 دقیقه

def verify_required_channels(user_id: int) -> bool:
    """FIX: cache - هر پیام یه API call نمیزنه"""
    now_ts = time.time()
    cached = _channel_verify_cache.get(user_id)
    if cached and (now_ts - cached[1]) < _CHANNEL_CACHE_TTL:
        return cached[0]

    channels = get_required_channels()
    if not channels:
        _channel_verify_cache[user_id] = (True, now_ts)
        return True
    ok_all = True
    for ch in channels:
        try:
            ...
            ok_all = False
            break
    _channel_verify_cache[user_id] = (ok_all, now_ts)
    if ok_all:
        with get_db() as conn:
            conn.execute("UPDATE users SET channel_joined=1 WHERE user_id=?", (user_id,))
            conn.commit()
...
def _render_about_text() -> str:
    ...
    updated = get_setting("about_updated_at", "")
    suffix = f"\n\n🕒 آخرین بروزرسانی: `{updated[:19].replace('T',' ')}`" if updated else ""
...
def _mark_locked(scope: str, ref_id: int, decision: str, handled_by: int) -> bool:
    """FIX: BEGIN IMMEDIATE برای جلوگیری از race condition بین مدیران"""
    ...
    try:
        ...
        row = conn.execute(
            "SELECT decision FROM admin_request_locks WHERE scope=? AND ref_id=?",
            (scope, ref_id)
        ).fetchone()
        ...
        conn.execute(
            ...
            "WHERE admin_request_locks.decision IS NULL",
        ...
        try:
            ...
            pass
        log.error(f"_mark_locked error: {e}")
        return False

def _broadcast_to_all_users(text: str, parse_mode: str = "Markdown", media: dict = None, retries: int = 2):
    with get_db() as conn:
    ...
    with _TPE(max_workers=15) as _exc:
        _exc.map(worker, users)
    return ok, fail
...
def init_db():
    ...
    with get_db() as conn:
        ...
        conn.execute("CREATE TABLE IF NOT EXISTS bot_store_submissions (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, bot_id INTEGER, bot_name TEXT, bot_username TEXT DEFAULT NULL, display_name TEXT, promo_text TEXT, description TEXT, status TEXT DEFAULT 'pending', reviewed_by INTEGER DEFAULT NULL, reviewed_at TEXT DEFAULT NULL, created_at TEXT DEFAULT NULL)")
        conn.execute("CREATE TABLE IF NOT EXISTS featured_bots (id INTEGER PRIMARY KEY AUTOINCREMENT, submission_id INTEGER UNIQUE, user_id INTEGER, bot_id INTEGER, bot_name TEXT, bot_username TEXT DEFAULT NULL, display_name TEXT, promo_text TEXT, description TEXT, approved_by INTEGER, approved_at TEXT, created_at TEXT DEFAULT NULL)")
        # migration برای ستون bot_username روی DB های قدیمی
        try:
            conn.execute("ALTER TABLE bot_store_submissions ADD COLUMN bot_username TEXT DEFAULT NULL")
        except Exception:
            pass
        try:
            conn.execute("ALTER TABLE featured_bots ADD COLUMN bot_username TEXT DEFAULT NULL")
        except Exception:
            pass
...
def handle_admin_panel(query: dict):
    ...
    edit_message(chat_id, msg_id, "⚙️ *پنل مدیریت*\n\nانتخاب کنید:", reply_markup=inline_keyboard([
        [("📢 پیام همگانی", "broadcast_msg"), ("📰 مدیریت اخبار", "admin_news_menu")],
        [("ℹ️ مدیریت درباره ما", "admin_about_menu"), ("🤖 مدیریت ربات‌ها", "admin_bots_menu")],
        [("📦 مدیریت کتابخانه‌ها", "admin_libraries_menu"), ("👥 مدیریت کاربران", "user_management")],
        [("📊 آمار لحظه‌ای", "realtime_stats"), ("📈 آمار هفتگی", "weekly_stats")],
        [("✅ درخواست‌ها", "pending_requests"), ("🎟 کدهای تخفیف", "discount_panel")],
        [("💳 مدیریت اشتراک", "subscription_mgmt"), ("💳 توکن پرداخت", "payment_token_panel")],
        [("🎪 قرعه‌کشی", "lottery_admin"), ("🏆 چالش‌های کد", "challenge_admin")],
        [("🪄 مدیریت بازو استور", "admin_botstore_mgmt"), ("🎫 تیکت‌ها", "tickets_admin")],
        [("⚙️ تنظیمات", "admin_settings"), ("🔙 بازگشت", "main_menu")]
    ]))
...
def handle_admin_settings(query: dict):
    ...
    edit_message(chat_id, msg_id,
        ...
        reply_markup=inline_keyboard([
            ...
            [("💰 تغییر قیمت‌ها", "change_prices"), ("ℹ️ درباره ما", "admin_about_menu")],
            [("📰 اخبار", "admin_news_menu"), ("🔙 پنل مدیریت", "admin_panel")]
        ])
    )
...
def handle_broadcast_content(message: dict):
    ...
    with get_db() as conn:
        ...
        conn.commit()
    ok, fail = _broadcast_to_all_users(text, parse_mode="Markdown", media=media, retries=2)
...
def handle_bot_store_description(message: dict):
    ...
    # NEW: مرحله دریافت آیدی بازو قبل از تأیید
    set_state(user_id, "bot_store_bot_username", data)
    send_message(
        message["chat"]["id"],
        "🤖 *مرحله آخر:* آیدی بازو خود را ارسال کنید.\n\n"
        "مثال: `@my_awesome_bot`\n\n"
        "⚠️ آیدی باید با `@` شروع و با `bot` ختم شود.",
        reply_markup=inline_keyboard([[("❌ انصراف", "bot_store")]])
    )
    return True


def handle_bot_store_bot_username(message: dict):
    """NEW: دریافت و اعتبارسنجی آیدی یوزرنیم بازو"""
    user_id = message["from"]["id"]
    if get_state(user_id)["state"] != "bot_store_bot_username":
        return False
    chat_id = message["chat"]["id"]
    username = (message.get("text") or "").strip()
    ...
    if not username.lower().endswith("bot"):
        send_message(chat_id,
            "❌ آیدی باید با `bot` ختم شود.\n"
            "مثال: `@my_awesome_bot`\n\n"
            "دوباره ارسال کنید:")
        return True
    if len(username) < 6:
        send_message(chat_id, "❌ آیدی خیلی کوتاه است. دوباره ارسال کنید:")
        return True
    ...
    confirm_text = (
        f"🪄 *تأیید کاندید بازو*\n\n"
        f"ربات (فایلی): `{data.get('bot_name')}`\n"
        f"🔗 آیدی بازو: `{username}`\n"
        f"📌 اسم نمایشی: `{data.get('display_name')}`\n"
        f"📢 متن تبلیغاتی: {data.get('promo_text')}\n"
        f"📝 توضیح: {data.get('description')}\n\n"
        f"همه چیز درسته؟ تأیید کنید:"
    )
    send_message(chat_id, confirm_text, reply_markup=inline_keyboard([
        [("✅ تایید و ارسال به مدیر", "bot_store_submit")],
        [("❌ انصراف", "bot_store")]
    ]))
    return True
...
def handle_bot_store_submit(query: dict):
    ...
    with get_db() as conn:
        ...
        if not bot:
            ...
            return
        conn.execute(
            "INSERT INTO bot_store_submissions (user_id, bot_id, bot_name, bot_username, display_name, promo_text, description, status, created_at) VALUES (?,?,?,?,?,?,?,?,?)",
            (user_id, bot_id, bot["bot_name"], data.get("bot_username", ""), data["display_name"], data["promo_text"], data["description"], "pending", datetime.now().isoformat())
        )
        req_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
        conn.commit()

    bot_username_display = data.get("bot_username") or f"@{bot['bot_name'].lstrip('@')}"
    req_text = f"""🪄 *درخواست بازو استور*

کاربر: `{user_id}`
ربات (فایلی): `{bot['bot_name']}`
🔗 آیدی بازو: `{bot_username_display}`
...
def handle_botstore_approve(query: dict, req_id: int):
    ...
    with get_db() as conn:
        ...
        conn.execute(
            "INSERT INTO featured_bots (submission_id, user_id, bot_id, bot_name, bot_username, display_name, promo_text, description, approved_by, approved_at, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            (req_id, row["user_id"], row["bot_id"], row["bot_name"], row.get("bot_username") or row["bot_name"], row["display_name"], row["promo_text"], row["description"], admin_id, datetime.now().isoformat(), datetime.now().isoformat())
        )
        conn.commit()
    try:
        bot_link = row.get("bot_username") or f"@{row['bot_name'].lstrip('@')}"
        if not bot_link.startswith("@"):
            bot_link = "@" + bot_link
        ok, fail = _broadcast_to_all_users(f"""⭐ *بازوی منتخب جدید*

{row['promo_text']}

🤖 بازو: {bot_link}""", retries=2)
    except Exception:
        ok, fail = 0, 0
...
def handle_callback_query(update: dict):
    ...
    if data == "admin_botstore_mgmt":
        handle_admin_botstore_mgmt(query); return
    if data.startswith("admin_bs_list:"):
        handle_admin_bs_list(query, int(data.split(":")[1])); return
    if data.startswith("admin_bs_detail:"):
        handle_admin_bs_detail(query, int(data.split(":")[1])); return
    if data.startswith("admin_bs_delete_confirm:"):
        handle_admin_bs_delete_confirm(query, int(data.split(":")[1])); return
    if data.startswith("admin_bs_delete:"):
        handle_admin_bs_delete(query, int(data.split(":")[1])); return
    if data.startswith("admin_bs_edit_name:"):
        handle_admin_bs_edit_field(query, int(data.split(":")[1]), "display_name"); return
    if data.startswith("admin_bs_edit_promo:"):
        handle_admin_bs_edit_field(query, int(data.split(":")[1]), "promo_text"); return
    if data.startswith("admin_bs_edit_desc:"):
        handle_admin_bs_edit_field(query, int(data.split(":")[1]), "description"); return
    if data.startswith("admin_bs_edit_username:"):
        handle_admin_bs_edit_field(query, int(data.split(":")[1]), "bot_username"); return
    if data == "tickets_admin":
        handle_tickets_admin(query); return
...
def handle_message(update):
    ...
    if handle_bot_store_description(message):
        return
    if handle_bot_store_bot_username(message):
        return
    ...
    if st in ("deploy_waiting_file", "deploy_confirm", "install_lib_name", "broadcast_waiting_msg",
              ...
              "admin_bs_edit_name", "admin_bs_edit_promo", "admin_bs_edit_desc"):
...
# ═══════════════════════════════════════════════════════════
#  مدیریت بازو استور - Admin Bot Store Management
# ═══════════════════════════════════════════════════════════
...
def handle_admin_botstore_mgmt(query: dict, page: int = 0):
    admin_id = query["from"]["id"]
    chat_id = query["message"]["chat"]["id"]
    msg_id = query["message"]["message_id"]
    if not is_admin(admin_id):
    ...
    PAGE_SIZE = 8
    with get_db() as conn:
        total = conn.execute("SELECT COUNT(*) as c FROM featured_bots").fetchone()["c"]
        rows = conn.execute(
            "SELECT fb.*, u.full_name FROM featured_bots fb "
            "LEFT JOIN users u ON fb.user_id=u.user_id "
            "ORDER BY fb.approved_at DESC LIMIT ? OFFSET ?",
            (PAGE_SIZE, page * PAGE_SIZE)
        ).fetchall()
    text = f"🪄 *مدیریت بازو استور*\n\nتعداد کل: {total} بازو | صفحه {page + 1}"
    buttons = [[(f"⭐ {r['display_name']}", f"admin_bs_detail:{r['id']}")] for r in rows]
    nav = []
    if page > 0:
        nav.append(("◀️ قبلی", f"admin_bs_list:{page - 1}"))
    if (page + 1) * PAGE_SIZE < total:
        nav.append(("بعدی ▶️", f"admin_bs_list:{page + 1}"))
    if nav:
        buttons.append(nav)
    buttons.append([("🔙 پنل مدیریت", "admin_panel")])
    edit_message(chat_id, msg_id, text, reply_markup=inline_keyboard(buttons))
...
def handle_admin_bs_list(query: dict, page: int):
    handle_admin_botstore_mgmt(query, page)


def handle_admin_bs_detail(query: dict, fid: int):
    admin_id = query["from"]["id"]
    chat_id = query["message"]["chat"]["id"]
    msg_id = query["message"]["message_id"]
    if not is_admin(admin_id):
    ...
    with get_db() as conn:
        row = conn.execute(
            "SELECT fb.*, u.full_name FROM featured_bots fb "
            "LEFT JOIN users u ON fb.user_id=u.user_id WHERE fb.id=?",
            (fid,)
        ).fetchone()
    if not row:
        answer_callback(query["id"], "بازو یافت نشد!", True)
        return
    row = dict(row)
    bot_link = row.get("bot_username") or f"@{row['bot_name'].lstrip('@')}"
    if not bot_link.startswith("@"):
        bot_link = "@" + bot_link
    owner = row.get("full_name") or str(row["user_id"])
    approved_date = str(row.get("approved_at", ""))[:10]
    text = (
        f"⭐ *{row['display_name']}*\n\n"
        f"👤 صاحب: {owner}\n"
        f"🤖 نام فایلی: `{row['bot_name']}`\n"
        f"🔗 آیدی بازو: `{bot_link}`\n"
        f"📢 متن تبلیغ: {row['promo_text'][:100]}\n"
        f"📝 توضیح: {row['description'][:100]}\n"
        f"📅 تایید شده: {approved_date}"
    )
    edit_message(chat_id, msg_id, text, reply_markup=inline_keyboard([
        [("✏️ اسم نمایشی", f"admin_bs_edit_name:{fid}"), ("✏️ متن تبلیغ", f"admin_bs_edit_promo:{fid}")],
        [("✏️ توضیح", f"admin_bs_edit_desc:{fid}"), ("✏️ آیدی بازو", f"admin_bs_edit_username:{fid}")],
        [("🗑 حذف از استور", f"admin_bs_delete_confirm:{fid}")],
        [("🔙 لیست بازو استور", "admin_botstore_mgmt")]
    ]))


def handle_admin_bs_delete_confirm(query: dict, fid: int):
    admin_id = query["from"]["id"]
    chat_id = query["message"]["chat"]["id"]
    msg_id = query["message"]["message_id"]
    if not is_admin(admin_id):
        return
    with get_db() as conn:
        row = conn.execute("SELECT display_name FROM featured_bots WHERE id=?", (fid,)).fetchone()
    name = row["display_name"] if row else f"#{fid}"
    edit_message(
        chat_id, msg_id,
        f"⚠️ آیا مطمئنید می‌خواهید *{name}* را از بازو استور حذف کنید؟\n\nاین عمل غیرقابل بازگشت است.",
        reply_markup=inline_keyboard([
            [("✅ بله، حذف شود", f"admin_bs_delete:{fid}")],
            [("❌ انصراف", f"admin_bs_detail:{fid}")]
        ])
    )


def handle_admin_bs_delete(query: dict, fid: int):
    admin_id = query["from"]["id"]
    chat_id = query["message"]["chat"]["id"]
    msg_id = query["message"]["message_id"]
    if not is_admin(admin_id):
    ...
    with get_db() as conn:
        row = conn.execute("SELECT * FROM featured_bots WHERE id=?", (fid,)).fetchone()
        conn.execute("DELETE FROM featured_bots WHERE id=?", (fid,))
        if row and row["submission_id"]:
            conn.execute(
                "UPDATE bot_store_submissions SET status='removed' WHERE id=?",
                (row["submission_id"],)
            )
        conn.commit()
    if row:
        try:
            send_message(
                row["user_id"],
                f"ℹ️ بازوی *{row['display_name']}* از بازو استور حذف شد.\n"
                "می‌توانید بازوی دیگری کاندید کنید."
            )
        except Exception:
            pass
        _notify_admins(
            f"🗑 بازوی *{row['display_name']}* (#{fid}) توسط مدیر `{admin_id}` از استور حذف شد.",
            exclude=admin_id
        )
    edit_message(
        chat_id, msg_id,
        "✅ بازو از استور حذف شد.",
        reply_markup=inline_keyboard([[("🔙 مدیریت بازو استور", "admin_botstore_mgmt")]])
    )


def handle_admin_bs_edit_field(query: dict, fid: int, field: str):
    """شروع ویرایش یک فیلد از بازوی استور"""
    admin_id = query["from"]["id"]
    chat_id = query["message"]["chat"]["id"]
    if not is_admin(admin_id):
        return
    state_map = {
        "display_name": "admin_bs_edit_name",
        "promo_text": "admin_bs_edit_promo",
        "description": "admin_bs_edit_desc",
        "bot_username": "admin_bs_edit_username"
    }
    label_map = {
        "display_name": "اسم نمایشی جدید",
        "promo_text": "متن تبلیغاتی جدید",
        "description": "توضیح جدید",
        "bot_username": "آیدی بازو جدید (مثال: @my_bot)"
    }
    st_key = state_map.get(field, "admin_bs_edit_name")
    label = label_map.get(field, "مقدار جدید")
    set_state(admin_id, st_key, {"fid": fid, "field": field})
    send_message(
        chat_id,
        f"✏️ *{label}* را ارسال کنید:",
        reply_markup=inline_keyboard([[("❌ انصراف", f"admin_bs_detail:{fid}")]])
    )
